...
import { MdAttachMoney, MdCheckCircle, MdPending, MdPerson, MdAccountBalanceWallet, MdCalendarToday, MdReceiptLong } from 'react-icons/md';
...
const StudentFees = () => {
  ...
  const calculateStats = (feesList) => {
    const totals = feesList.reduce((acc, fee) => {
      const amount = parseFloat(fee?.amount || 0);
  ...
  const getStatusIcon = (status) => {
    ...
    if (statusLower === 'pending' || statusLower === 'overdue') return <MdPending />;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl p-6 shadow-md text-center">
            ...
            <p className="text-xl font-bold text-gray-900 mb-1">₹{stats.totalAmount.toLocaleString()}</p>
            <p className="text-xs text-gray-600 font-semibold">Total Fees</p>
          </div>
          <div className="bg-white rounded-xl p-6 shadow-md text-center">
            <MdCheckCircle className="w-5 h-5 mx-auto text-green-500 mb-2" />
            <p className="text-xl font-bold text-green-600 mb-1">₹{stats.paidAmount.toLocaleString()}</p>
            <p className="text-xs text-gray-600 font-semibold">Paid</p>
          </div>
          <div className="bg-white rounded-xl p-6 shadow-md text-center">
            <MdPending className="w-5 h-5 mx-auto text-blue-500 mb-2" />
            <p className="text-xl font-bold text-blue-600 mb-1">₹{stats.pendingAmount.toLocaleString()}</p>
            ...
            {fees.map((fee, index) => (
              <div key={index} className="bg-white rounded-xl shadow-md p-4">
                ...
                <div className="flex items-center justify-between pb-3 mb-3 border-b border-gray-100">
                  <div>
                    <p className="text-lg font-bold text-gray-900">{fee.course || 'Course'}</p>
                    <p className="text-xs text-gray-600 font-medium">ID: {fee.feeId || 'N/A'}</p>
                  </div>
                  <div 
                    className="flex items-center gap-1 px-3 py-1.5 rounded-xl"
                    style={{ backgroundColor: getStatusColor(fee.status) }}
                  >
                    <span className="text-white text-sm">{getStatusIcon(fee.status)}</span>
                    <span className="text-xs font-bold text-white">{fee.status || 'Pending'}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2 mb-3">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-gray-600 font-medium flex-1">Student:</span>
                    <span className="text-sm text-gray-900 font-semibold">{fee.studentName || 'N/A'}</span>
                  </div>
                  
                  <div className="flex items-center">
                    <MdAttachMoney className="w-4 h-4 text-gray-600 mr-2" />
                    <span className="text-sm text-gray-600 font-medium flex-1">Total:</span>
                    <span className="text-sm text-gray-900 font-semibold">₹{parseFloat(fee.amount || 0).toLocaleString()}</span>
                  </div>
                  
                  <div className="flex items-center">
                    <MdAccountBalanceWallet className="w-4 h-4 text-green-500 mr-2" />
                    <span className="text-sm text-gray-600 font-medium flex-1">Paid:</span>
                    <span className="text-sm text-green-600 font-semibold">₹{parseFloat(fee.paidAmount || 0).toLocaleString()}</span>
                  </div>

                  {(fee.pendingAmount || 0) > 0 && (
                    <div className="flex items-center">
                      <MdPending className="w-4 h-4 text-blue-500 mr-2" />
                      <span className="text-sm text-gray-600 font-medium flex-1">Pending:</span>
                      <span className="text-sm text-blue-600 font-semibold">₹{parseFloat(fee.pendingAmount || 0).toLocaleString()}</span>
                    </div>
                  )}
                  
                  <div className="flex items-center">
                    <MdCalendarToday className="w-4 h-4 text-gray-600 mr-2" />
                    <span className="text-sm text-gray-600 font-medium flex-1">Due Date:</span>
                    <span className="text-sm text-gray-900 font-semibold">{formatDate(fee.dueDate)}</span>
                  </div>
                </div>
              </div>
            ))}
