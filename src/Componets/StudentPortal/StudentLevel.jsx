...
const StudentLevel = () => {
  ...
  const loadLevelData = async () => {
    try {
      ...
      const promotionsResponse = await fetch(`${API_BASE_URL}/student-portal/promotions`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      try {
        console.log('🔄 Fetching upcoming tests for logged-in student from:', `${API_BASE_URL}/student-portal/upcoming-tests`);
        const testsResponse = await fetch(`${API_BASE_URL}/student-portal/upcoming-tests`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const getBeltColor = (beltName) => {
    ...
    if (belt.includes('yellow')) return 'bg-yellow-400 border-yellow-600 text-gray-800';
    if (belt.includes('green')) return 'bg-green-500 border-green-700 text-white';
    if (belt.includes('blue')) return 'bg-blue-500 border-blue-700 text-white';
    if (belt.includes('purple')) return 'bg-purple-500 border-purple-700 text-white';
        ...
        <div className="bg-white rounded-xl shadow-md p-2 mb-6 grid grid-cols-3 gap-2">
          <button
            ...
            className={`px-4 py-3 rounded-lg font-semibold transition-all ${
              viewMode === 'belt-levels' ? 'bg-[#006CB5] text-white' : 'text-gray-600 hover:bg-gray-100'
            }`}
          >
            Belt Levels
          </button>
          <button
            ...
            className={`px-4 py-3 rounded-lg font-semibold transition-all ${
              viewMode === 'promotions' ? 'bg-[#006CB5] text-white' : 'text-gray-600 hover:bg-gray-100'
            }`}
          >
            Promotions
          </button>
          <button
            ...
            className={`px-4 py-3 rounded-lg font-semibold transition-all ${
              viewMode === 'upcoming' ? 'bg-[#006CB5] text-white' : 'text-gray-600 hover:bg-gray-100'
            }`}
          >
            Upcoming
          </button>
        ...
        {viewMode === 'belt-levels' ? (
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="p-4 bg-gray-50 border-b border-gray-200">
              <h3 className="text-lg font-bold text-gray-900">All Belt Levels</h3>
            </div>
            
            {beltLevels.length === 0 ? (
              <div className="p-12 text-center">
                <FaMedal className="w-16 h-16 mx-auto text-gray-400 mb-4" />
                <p className="text-gray-600 text-lg">No belt levels available</p>
              </div>
            ) : (
              <div className="divide-y divide-gray-200">
                {beltLevels.map((belt, index) => (
                  <div key={index} className="transition-colors">
                          ...
                          {isStripeBelt(belt.name) ? (
                            ...
                            <div className="w-16 h-16 rounded-full border-2 border-gray-800 flex overflow-hidden">
                              <div 
                                ...
                                style={{ 
                                  backgroundColor: getStripeColors(belt.name)?.color1,
                                  width: '70%'
                                }}
                              >
                              </div>
                              <div 
                                className="flex items-center justify-center"
                                style={{ 
                                  backgroundColor: getStripeColors(belt.name)?.color2,
                                  width: '30%'
                                }}
                              >
                              </div>
                            ...
                            >
                              <span className={`text-xs font-bold drop-shadow ${belt.hex ? getBeltTextColor(belt.hex) : ''}`}>
                                {belt.name?.split(' ')[0]}
                              </span>
                            </div>
                          )}
                          <div>
                            <p className="font-semibold text-gray-900">{belt.name}</p>
                            <p className="text-sm text-gray-600">Level {belt.level}</p>
                            {belt.requirements && belt.requirements.length > 0 && (
                              <p className="text-xs text-gray-500 mt-1">{belt.requirements.length} requirements</p>
                            )}
                          </div>
                        </div>
                    ...
                    {expandedBelt === index && belt.requirements && belt.requirements.length > 0 && (
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="p-4 bg-gray-50 border-b border-gray-200">
              <h3 className="text-lg font-bold text-gray-900">Upcoming Belt Tests</h3>
            </div>
                  ...
                  <div key={index} className="p-4 hover:bg-gray-50 transition-colors">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <p className="font-semibold text-gray-900">{test.studentName}</p>
                        <div className="flex items-center space-x-2 mt-1">
                          <span className="text-sm text-gray-600">{test.currentBelt}</span>
                          <span className="text-gray-400">→</span>
                          <span className="text-sm font-semibold text-[#006CB5]">{test.testingFor}</span>
                        </div>
                        <p className="text-xs text-gray-500 mt-1">{formatDate(test.testDate)}</p>
                        {test.certificateCode && (
                          <p className="text-xs text-gray-500 mt-1">Certificate: {test.certificateCode}</p>
                        )}
                      </div>
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => {
                              ...
                              const filePath = test.certificateFile.startsWith('/') ? test.certificateFile : `/${test.certificateFile}`;
                            ...
                            style={{ color: '#ffffff' }}
                          >
                            <FaCheckCircle className="w-4 h-4" style={{ color: '#ffffff' }} />
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            ...
            {promotions.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-gray-600 text-lg">No promotion history yet</p>
              </div>
            ) : (
              <div className="divide-y divide-gray-200">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        {isStripeBelt(promo.toBelt) ? (
                          ...
                          <div className="w-16 h-16 rounded-full border-2 border-gray-800 flex overflow-hidden">
                            <div 
                              className="flex items-center justify-center"
                              style={{ 
                                backgroundColor: getStripeColors(promo.toBelt)?.color1,
                                width: '70%'
                              }}
                            >
                            </div>
                            <div 
                              className="flex items-center justify-center"
                              style={{ 
                                backgroundColor: getStripeColors(promo.toBelt)?.color2,
                                width: '30%'
                              }}
                            >
                            </div>
                          ...
                          // Solid color belt
                          <div className={`w-16 h-16 rounded-full border-2 flex items-center justify-center ${getBeltColor(promo.toBelt)}`}>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{formatDate(promo.promotionDate)}</p>
                        </div>
                      </div>
