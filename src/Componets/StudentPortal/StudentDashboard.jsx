...
import { FaUser, FaBell, FaSignOutAlt, FaMoneyBillWave, FaCalendarAlt, FaCheckCircle, FaMedal, FaCertificate } from 'react-icons/fa';
...
const StudentDashboard = () => {
    ...
    try {
      ...
      if (profileResponse.ok) {
        ...
        if (profileData.status === 'success') {
          setStudentName(profileData.data.fullName || profileData.data.name || 'Student');
          setCurrentBeltLevel(profileData.data.currentBeltLevel || 'N/A');
        }
      }

      // Fetch attendance data
      const attendanceResponse = await fetch(`${API_BASE_URL}/student-portal/attendance`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
        ...
        if (attendanceData.status === 'success') {
          ...
          if (records.length > 0) {
            const presentCount = records.filter(r => 
              r.status && (r.status.toLowerCase() === 'present' || r.status.toLowerCase() === 'late')
            ).length;
      ...
      if (feesResponse.ok) {
        ...
        if (feesData.status === 'success') {
          ...
          const pending = fees.filter(f => 
            f.status && (f.status.toLowerCase() === 'pending' || f.status.toLowerCase() === 'overdue')
          ).length;
      ...
      // Fetch events data
      const eventsResponse = await fetch(`${API_BASE_URL}/student-portal/events`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const quickAccess = [
    { title: 'Attendance', Icon: FaCheckCircle, color: '#e74c3c', route: '/student/attendance' },
    { title: 'Level/Belt', Icon: FaMedal, color: '#e74c3c', route: '/student/level' },
    { title: 'Events', Icon: FaCalendarAlt, color: '#e74c3c', route: '/student/events' },
    { title: 'Certificates', Icon: FaCertificate, color: '#e74c3c', route: '/student/certificates' },
    { title: 'Fees', Icon: FaMoneyBillWave, color: '#e74c3c', route: '/student/fees' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <p className="text-sm text-white opacity-90">Hello!</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {quickOverview.map((item, index) => (
              ...
              >
                ...
                <p className="text-3xl font-bold text-gray-900 text-center mb-1">{item.title}</p>
                <p className="text-sm text-gray-600 font-semibold text-center">{item.subtitle}</p>
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {quickAccess.map((item, index) => (
              ...
              >
                ...
                <p className="text-sm text-gray-900 font-semibold text-center">{item.title}</p>
              </button>
            ))}
          </div>
        </div>
