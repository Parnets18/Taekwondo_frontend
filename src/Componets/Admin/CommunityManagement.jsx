...
function CommunityManagement() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.role || !formData.belt) {
    ...
    try {
      ...
      const url = selectedMember
        ? `${API_BASE_URL}/community/${selectedMember._id}`
  ...
  const handleDelete = async (memberId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE_URL}/community/${memberId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {members.length === 0 ? (
          <div className="text-center py-20 bg-white">
            <h3 className="text-2xl font-semibold text-gray-700 mb-2">No Members Yet</h3>
            <p className="text-gray-500 mb-6">Start by adding your first community member</p>
            <button
              onClick={openAddModal}
                      ...
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-20 w-20">
                          {member.photo ? (
                            ...
                            <div className="h-20 w-20 rounded-lg bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 bg-white">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="e.g., Head Instructor, Senior Instructor"
                  required
                />
              </div>
      ...
      {showViewModal && selectedMember && (
        <div className="fixed inset-0 bg-transparent z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900">View Member</h2>
              <button
                onClick={() => {
