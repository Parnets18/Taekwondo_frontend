    ...
    <div className="space-y-6">
      <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-200">
        <div className="flex items-center justify-between">
          <div><h1 className="text-3xl font-bold" style={{ color: '#006CB5' }}>Dashboard Overview</h1><p className="mt-2" style={{ color: '#000000', fontSize: '1rem' }}>Welcome back! Here is what is happening with your academy.</p></div>
          <div className="text-right"><p className="text-sm" style={{ color: '#666666' }}>Today</p><p className="text-xl font-semibold" style={{ color: '#000000' }}>{new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p></div>
        </div>
      </div>
        ...
        {mainStats.map((stat, index) => (
          <Link key={index} to={stat.link} className="bg-white rounded-xl p-6 shadow-md hover:shadow-xl transition-all duration-300 border border-slate-200 hover:border-slate-300 group">
            <div className="flex items-start justify-between">
              <div className="flex-1"><p className="text-sm font-medium mb-2" style={{ color: '#666666' }}>{stat.title}</p><p className="text-3xl font-bold mb-2" style={{ color: '#000000' }}>{stat.value}</p><p className="text-xs" style={{ color: '#999999' }}>{stat.change}</p></div>
              <div className="p-3 rounded-lg transition-colors" style={{ backgroundColor: '#e3f2fd' }}><stat.icon className="text-2xl" style={{ color: '#006CB5' }} /></div>
            </div>
          </Link>
        ))}
      </div>
        ...
        {secondaryStats.map((stat, index) => (
          <div key={index} className="bg-white rounded-xl p-6 shadow-md border border-slate-200">
            <div className="flex items-center space-x-4">
              <div className="p-4 rounded-lg" style={{ backgroundColor: '#e3f2fd' }}><stat.icon className="text-3xl" style={{ color: '#006CB5' }} /></div>
              <div><p className="text-sm" style={{ color: '#666666' }}>{stat.title}</p><p className="text-2xl font-bold" style={{ color: '#000000' }}>{stat.value}</p><p className="text-xs" style={{ color: '#999999' }}>{stat.subtitle}</p></div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl shadow-md border border-slate-200">
          <div className="p-6 border-b border-slate-200"><div className="flex items-center justify-between"><h2 className="text-xl font-bold flex items-center" style={{ color: '#000000' }}><FaChartLine className="mr-2" style={{ color: '#006CB5' }} />Recent Activity</h2><span className="text-xs" style={{ color: '#999999' }}>System Status</span></div></div>
          <div className="p-6"><div className="space-y-4">{recentActivities.length > 0 ? recentActivities.map((activity, index) => (
            <div key={index} className="flex items-start space-x-4 p-3 rounded-lg hover:bg-slate-50 transition-colors">
              <div className="p-2 rounded-lg flex-shrink-0" style={{ backgroundColor: '#e3f2fd' }}><activity.icon className="text-lg" style={{ color: '#006CB5' }} /></div>
              <div className="flex-1 min-w-0"><p className="text-sm font-semibold" style={{ color: '#000000' }}>{activity.title}</p><p className="text-xs mt-1" style={{ color: '#666666' }}>{activity.subtitle}</p><p className="text-xs mt-1" style={{ color: '#999999' }}>{activity.time}</p></div>
            </div>
          )) : (<div className="text-center py-8"><p style={{ color: '#666666' }}>No recent activity</p><p className="text-xs mt-2" style={{ color: '#999999' }}>Activity will appear here as you use the system</p></div>)}</div></div>
        </div>

        <div className="bg-white rounded-xl shadow-md border border-slate-200">
          <div className="p-6 border-b border-slate-200"><div className="flex items-center justify-between"><h2 className="text-xl font-bold flex items-center" style={{ color: '#000000' }}><FaCalendarAlt className="mr-2" style={{ color: '#006CB5' }} />Upcoming Events</h2><Link to="/admin/events" className="text-xs font-medium" style={{ color: '#006CB5' }}>View All</Link></div></div>
          <div className="p-6"><div className="space-y-4">{upcomingEvents.length > 0 ? upcomingEvents.map((event, index) => (
            <div key={index} className="p-4 rounded-lg border border-slate-200 hover:border-slate-300 hover:shadow-sm transition-all">
              <div className="flex items-center justify-between mb-3"><h3 className="font-semibold" style={{ color: '#000000' }}>{event.title}</h3><span className={`px-3 py-1 text-xs font-medium rounded-full ${event.statusColor}`}>{event.status}</span></div>
              <div className="flex items-center justify-between text-sm" style={{ color: '#666666' }}><div className="flex items-center"><FaCalendarAlt className="mr-2" style={{ color: '#999999' }} /><span>{event.date}</span></div><div className="flex items-center"><FaUsers className="mr-2" style={{ color: '#999999' }} /><span>{event.participants} capacity</span></div></div>
            </div>
          )) : (<div className="text-center py-8"><p style={{ color: '#666666' }}>No upcoming events</p><p className="text-xs mt-2" style={{ color: '#999999' }}>Create events to see them here</p></div>)}</div>
          {upcomingEvents.length > 0 && (<Link to="/admin/events" className="mt-4 block text-center py-3 bg-slate-100 hover:bg-slate-200 rounded-lg text-sm font-medium transition-colors" style={{ color: '#000000' }}>Manage All Events</Link>)}</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Link to="/admin/students" className="p-4 bg-slate-50 hover:bg-blue-50 rounded-lg transition-all text-center group border border-slate-200 hover:border-blue-200"><FaUsers className="text-3xl mx-auto mb-2 transition-colors" style={{ color: '#006CB5' }} /><p className="text-sm font-medium" style={{ color: '#000000' }}>Manage Students</p></Link>
          <Link to="/admin/attendance" className="p-4 bg-slate-50 hover:bg-blue-50 rounded-lg transition-all text-center group border border-slate-200 hover:border-blue-200"><FaUserCheck className="text-3xl mx-auto mb-2 transition-colors" style={{ color: '#006CB5' }} /><p className="text-sm font-medium" style={{ color: '#000000' }}>Mark Attendance</p></Link>
          <Link to="/admin/belts" className="p-4 bg-slate-50 hover:bg-blue-50 rounded-lg transition-all text-center group border border-slate-200 hover:border-blue-200"><FaGraduationCap className="text-3xl mx-auto mb-2 transition-colors" style={{ color: '#006CB5' }} /><p className="text-sm font-medium" style={{ color: '#000000' }}>Belt Management</p></Link>
          <Link to="/admin/events" className="p-4 bg-slate-50 hover:bg-blue-50 rounded-lg transition-all text-center group border border-slate-200 hover:border-blue-200"><FaCalendarAlt className="text-3xl mx-auto mb-2 transition-colors" style={{ color: '#006CB5' }} /><p className="text-sm font-medium" style={{ color: '#000000' }}>Create Event</p></Link>
        </div>
      </div>
    </div>
  );
