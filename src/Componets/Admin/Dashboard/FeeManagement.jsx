...
function FeeManagement() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          ...
          throw new Error(
            "Server error: There is a technical issue with fee creation. Please contact the administrator or try again later.",
          );
        }
  ...
  const handleRecordPayment = (record) => {
    ...
    const totalAmount =
      record.amount +
      (record.lateFee?.amount || 0) -
      (record.discount?.amount || 0);
  ...
  const handlePaymentSubmit = (e) => {
    ...
    const remainingBalance =
      totalAmount - (selectedRecord.totalPaidAmount || 0);

    // Validate payment amount
    if (paymentForm.amount > remainingBalance) {
      alert(
        `Error: Payment amount (₹${paymentForm.amount.toLocaleString()}) cannot exceed remaining balance (₹${remainingBalance.toLocaleString()})`,
      );
      return;
    }
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-600">
            Track and manage student fee payments digitally
          </p>
        </div>
        <button
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-200 text-center">
          ...
          >
            {statistics.totalAmount > 0
              ? Math.round(
                  (statistics.paidAmount / statistics.totalAmount) * 100,
                )
              : 0}
            %
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="text-left py-3 px-4 font-bold text-slate-800 text-sm">
                  Fee ID
                </th>
                <th className="text-left py-3 px-4 font-bold text-slate-800 text-sm">
                  Student
                  ...
                  >
                    <td className="py-3 px-4">
                      <span className="font-semibold text-slate-800 text-sm">
                        {record.feeId || record._id.slice(-6)}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4">
                      <div>
                        {(() => {
                          ...
                          const remainingBalance =
                            totalAmount - (record.totalPaidAmount || 0);
              ...
              {selectedRecord.totalPaidAmount > 0 && (
                <div className="mt-3 p-3 bg-green-100 rounded-lg">
                  <div className="text-sm text-green-700 font-medium">
                    Already Paid: ₹
                    {selectedRecord.totalPaidAmount.toLocaleString()}
                  </div>
                  <div className="text-xs text-green-600">
                    {selectedRecord.status === "Paid"
                      ? "Fee is fully paid. You can add additional payments if needed."
                      : "This is a partial payment. You can pay the remaining amount or add more."}
                  </div>
                </div>
              )}
            </div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {paymentForm.lateFee.amount > 0 && (
                    <div className="flex justify-between text-red-600">
                      ...
                      <span className="font-semibold">
                        ₹{paymentForm.lateFee.amount.toLocaleString()}
                      </span>
                    </div>
                  )}
                  {paymentForm.discount.amount > 0 && (
                    <div className="flex justify-between text-green-600">
                      ...
                      <span className="font-semibold">
                        ₹{paymentForm.discount.amount.toLocaleString()}
                      </span>
                    </div>
                  )}
            ...
            <form onSubmit={handleAddFee} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                        ...
                        {studentSuggestions.length > 0 ? (
                          ...
                          <div className="px-3 py-4 text-center text-red-600">
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                              No student matches "{newFeeForm.studentName}"
                            </p>
                          </div>
                        )}
                      </div>
                  ...
                  {newFeeForm.studentName &&
                    !students.find(
                      (s) => s.fullName === newFeeForm.studentName,
                    ) &&
                    !showStudentSuggestions && (
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (newFeeForm.feeType === "Registration Fee")
                        amount = 500;
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (feeType === "Registration Fee") amount = 500;
                        ...
                        else if (newFeeForm.course === "Intermediate")
                          amount = 2500;
                        else if (newFeeForm.course === "Advanced")
                          amount = 3000;
                      }
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setNewFeeForm({ ...newFeeForm, notes: e.target.value })
                  }
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">
                      {newFeeForm.studentName || "Not specified"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">
                      ₹{newFeeForm.amount.toLocaleString()}
                    </span>
                  </div>
                  {newFeeForm.discount.amount > 0 && (
                    <div className="flex justify-between text-green-600">
                      <span>Discount:</span>
                      <span className="font-semibold">
                        -₹{newFeeForm.discount.amount.toLocaleString()}
                      </span>
                    </div>
                  )}
                  <div className="border-t border-blue-300 pt-2 mt-2">
                    <div className="flex justify-between font-bold">
                      ...
                      <span className="text-slate-800">
                        ₹
                        {(
                          newFeeForm.amount - (newFeeForm.discount.amount || 0)
                        ).toLocaleString()}
                      </span>
                    </div>
                  </div>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-lg font-semibold text-slate-800">
                      {selectedRecord.feeId || selectedRecord._id.slice(-6)}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-blue-600 font-medium">
                  ...
                  {selectedRecord.lateFee?.amount > 0 && (
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg font-semibold text-red-600">
                        +₹{selectedRecord.lateFee.amount.toLocaleString()}
                      </span>
                    </div>
                  )}
                  ...
                  {selectedRecord.discount?.amount > 0 && (
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg font-semibold text-green-600">
                        -₹{selectedRecord.discount.amount.toLocaleString()}
                      </span>
                    </div>
                  )}
                  ...
                  {(selectedRecord.status === "Paid" ||
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xl font-bold text-blue-600">
                        ₹
                        {(
                          selectedRecord.totalPaidAmount ||
                          selectedRecord.amount +
                            (selectedRecord.lateFee?.amount || 0) -
                            (selectedRecord.discount?.amount || 0)
                        ).toLocaleString()}
          ...
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl border-2 border-black">
            <div className="text-center mb-8">
              <div
                ...
                style={{
                  background: "linear-gradient(to right, #006CB5, #0056A3)",
                }}
              >
              ...
              <p className="text-slate-600 mt-2">
                Please login to access fee management system
              </p>
            </div>
            ...
            >
              ...
              <button
                ...
                style={{ backgroundColor: "#006CB5" }}
                onMouseEnter={(e) =>
                  (e.target.style.backgroundColor = "#0056A3")
                }
                onMouseLeave={(e) =>
                  (e.target.style.backgroundColor = "#006CB5")
                }
              >
                Login to Dashboard
              </button>
