...
import { FaPlus, FaEdit, FaTrash, FaTimes, FaArrowUp, FaArrowDown, FaEye, FaChevronLeft, FaChevronRight } from 'react-icons/fa';
...
export default function PracticeSuitManagement() {
  ...
  const paged = useMemo(() => items.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE), [items, safePage]);

  useEffect(() => { fetchItems(); }, []);

  const fetchItems = async () => {
    setLoading(true);
    const res = await fetch(`${API_BASE}/practice-suit`);
  ...
  const save = async () => {
    ...
    const url = editing ? `${API_BASE}/practice-suit/${editing._id}` : `${API_BASE}/practice-suit`;
  ...
  const del = async () => {
    await fetch(`${API_BASE}/practice-suit/${deleteId}`, { method: 'DELETE', headers: authH() });
    setDeleteId(null);
    fetchItems();
  };
  ...
  const moveItem = async (item, dir) => {
    ...
    await Promise.all([
      fetch(`${API_BASE}/practice-suit/${item._id}`, { method: 'PUT', headers: jsonH(), body: JSON.stringify({ order: swap.order }) }),
      fetch(`${API_BASE}/practice-suit/${swap._id}`, { method: 'PUT', headers: jsonH(), body: JSON.stringify({ order: item.order }) }),
    ]);
    fetchItems();
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="text-lg font-bold text-gray-800">Practice Suit (Do Bok)</h3>
          <p className="text-sm text-gray-500 mt-0.5">Manage content shown in the Practice Suit screen of the mobile app.</p>
        </div>
          ...
          <div className="bg-white rounded-xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="e.g. Meaning of belt colors"
                  value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
              </div>

              <div>
                <label className="text-sm font-semibold text-gray-700 block mb-1">Subtitle</label>
                <input className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="e.g. Wreathe the belt"
                  value={form.subtitle} onChange={e => setForm({ ...form, subtitle: e.target.value })} />
              </div>

              <div>
                <label className="text-sm font-semibold text-gray-700 block mb-1">Description</label>
                <textarea rows={3} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Introductory text..."
          ...
          <div className="bg-white rounded-xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-800">{viewItem.title || '—'}</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Subtitle</p>
                <p className="text-sm text-gray-800">{viewItem.subtitle || '—'}</p>
              </div>
              <div>
